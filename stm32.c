...
static stm32_err_t stm32_send_init_seq(const stm32_t *stm)
{
	...
	//work around for stm32F777 device replying with 0x00 response 
	//need to send 2 more init requests 1 will timeout and one should get NACK
	if (p_err == PORT_ERR_OK && byte == 0) {
		fprintf(stderr, "Received 0x00 response, resending request\n");
		p_err = port->write(port, &cmd, 1);
		if (p_err != PORT_ERR_OK) {
			fprintf(stderr, "Failed to send init to device\n");
			return STM32_ERR_UNKNOWN;
	...
	if ((p_err != PORT_ERR_TIMEDOUT) && (byte != 0)) {
		fprintf(stderr, "Failed to init device. Unknown Error\n");
		return STM32_ERR_UNKNOWN;
	}
